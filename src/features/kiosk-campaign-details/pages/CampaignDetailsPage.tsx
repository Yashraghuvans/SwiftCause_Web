...
export const CampaignDetailsPage: React.FC<CampaignDetailsPageProps> = ({
  ...
  // Loading state
  if (loading) {
    return <LoadingState accentColorHex={accentColorHex} organizationId={organizationId} />;
  }
  ...
  const recurringIntervals = (
    campaign.configuration?.recurringIntervals?.length
      ? campaign.configuration.recurringIntervals
      : ['monthly', 'quarterly', 'yearly']
  ) as ('monthly' | 'quarterly' | 'yearly')[];
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white relative font-lexend">
      ...
      <button
        onClick={onBack}
        className="absolute left-5 top-4 z-30 inline-flex items-center gap-2 text-sm font-medium hover:underline underline-offset-4"
      ...
      <div className="relative z-10 flex flex-col min-h-screen">
          ...
          <div className="grid grid-cols-5 gap-4 h-full w-full items-start">
            ...
            <div className="col-span-2 rounded-[18px] border border-gray-200/50 bg-[#FFFBF7] shadow-[0_10px_28px_rgba(15,23,42,0.08)] px-4 py-3.5 lg:sticky lg:top-0 h-fit pt-8">
              <div className="flex flex-col gap-3 max-h-[calc(100vh-120px)] overflow-y-auto pr-1 hide-scrollbar">
                ...
                <div>
                  <p className="text-[13px] font-medium text-slate-500 uppercase tracking-wider mb-1.5">
                    Choose an amount
                  </p>
                  <AmountSelector
                ...
                <div className="space-y-1">
                  <DonateButton
                    ...
                    accentColorHex={accentColor}
                  />
                  <p className="text-[12px] text-center text-slate-400 font-normal">
        ...
        <div className="lg:hidden">
          ...
          <div className="px-4 pt-14" style={{ paddingBottom: `${donationPanelHeight}px` }}>
            {/* Image Carousel */}
            ...
            <div className="space-y-1.5 mb-4 rounded-xl border border-gray-200/50 bg-gray-100/50 px-3.5 py-2">
              <div className="flex justify-between items-center">
                ...
                <span className="text-[15px] font-medium" style={{ color: accentColor }}>
                  {Math.round(progress)}%
                </span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div
                  className="kiosk-progress-bar h-2 rounded-full transition-all duration-300 ease-out"
                  style={{ backgroundColor: accentColor, width: `${progress}%` }}
                />
              </div>
            </div>
