                ...
                {config.showProgressBar && campaign.goal > 0 && (
                  <div className="space-y-2 mb-4">
                    <div className="flex justify-between text-sm">
                      <span>
                        Raised: {formatCurrency(campaign.raised, organizationCurrency || 'GBP')}
                      </span>
                      <span>
                        Goal:{' '}
                        {formatCurrencyFromMajor(campaign.goal, organizationCurrency || 'GBP')}
                      </span>
                    </div>
                    <Progress
                      value={Math.min(
                        getProgressPercentage(campaign.raised || 0, campaign.goal || 0),
                        100,
                      )}
                      className="h-2"
                    />
                    <p className="text-sm text-gray-600">
                      {getProgressPercentage(campaign.raised || 0, campaign.goal || 0).toFixed(1)}%
                      funded
                    </p>
                  </div>
                )}
                ...
                {campaign.organizationInfo && (
                  <div className="space-y-3 mt-4 p-4 border rounded-lg bg-gray-50">
                    <h5 className="font-bold text-gray-800">
                      About {campaign.organizationInfo.name}
                    </h5>
                    ...
                    {campaign.organizationInfo.website && (
                      <a
                        href={campaign.organizationInfo.website}
                        target="_blank"
                            ...
                            <div
                              ...
                              dangerouslySetInnerHTML={{
                                __html:
                                  ...
                                  'This campaign is dedicated to making a significant impact. Your support will provide essential resources and foster lasting positive change within the community.',
                              }}
                            />
                            {campaign.videoUrl && (
                              ...
                              <div className="grid grid-cols-2 gap-2">
                                {campaign.galleryImages.map((img: string, idx: number) => (
                                  <ImageWithFallback
                                    key={idx}
                                    src={img}
                                    alt={`${campaign.title} gallery image ${idx + 1}`}
                              ...
                              <div className="space-y-2">
                                ...
                                {campaign.impactMetrics.peopleHelped && (
                                  <p className="flex items-center">
                                    ...
                                    {campaign.impactMetrics.peopleHelped.toLocaleString()} people
                                    helped
                                  </p>
                                )}
                                {campaign.impactMetrics.itemsProvided && (
                                  <p className="flex items-center">
                                    <Gift className="h-4 w-4 mr-2 text-indigo-600" />{' '}
                                    {campaign.impactMetrics.itemsProvided.toLocaleString()} items
                                    provided
                                  </p>
                                )}
                                {campaign.impactMetrics.customMetric && (
                                  <p className="flex items-center">
                                    ...
                                    {campaign.impactMetrics.customMetric.value.toLocaleString()}{' '}
                      ...
                      {config.predefinedAmounts.map((amount: number) => (
                        ...
                        >
                          <span className="text-lg font-semibold">
                            {formatCurrencyFromMajor(amount, organizationCurrency || 'GBP')}
                          </span>
                          {config.enableRecurring && isRecurring && config.recurringDiscount && (
                            <span className="text-xs opacity-75">
                              {formatCurrencyFromMajor(
                                amount * (1 - config.recurringDiscount / 100),
                    ...
                    {config.allowCustomAmount && (
                      <div className="space-y-2">
                        ...
                        <p className="text-sm text-gray-500">
                          Enter between{' '}
                          {formatCurrencyFromMajor(
                            config.minCustomAmount,
                            organizationCurrency || 'GBP',
                          )}{' '}
                          and{' '}
                          {formatCurrencyFromMajor(
                            config.maxCustomAmount,
                            organizationCurrency || 'GBP',
                          )}
                        </p>
                      </div>
